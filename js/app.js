...
App.factory('pageMetaService', ['$rootScope', '$log', '$location', function($rootScope, $log, $location) {
    var setTitle = function(newTitle) {
        $log.info("Set title: "+newTitle);
        if(newTitle === "") {
          $rootScope.title = 'Nobar';
        } else {
          $rootScope.title = newTitle;
        }

        // reset robots and prev/next link here. not sure of a better place to do this
        setMetaRobots();
        setRelPrevNextLink();
    };

    var setCanonicalUrl = function(url, pagePrefix, pagingService, hash) {
        var canonicalUrl = '';

        if(pagingService && pagingService.canonicalPage()) {
          canonicalUrl = url + pagePrefix + '/' + pagingService.canonicalPage();
        } else {
          canonicalUrl = url;
        }

        if(hash) {
          canonicalUrl = canonicalUrl + '#' + hash;
        }

        $rootScope.canonicalUrl = canonicalUrl;
        $log.info("Set canonical: "+canonicalUrl);
        // if ($location.host() === 'www.redbullsoundselect.com') {
        //   if (url) history.replaceState('','',url);
        // }
    };

    var setMetaDescription = function(newDescription) {
        $log.info("Set meta description: "+newDescription);
        $rootScope.metaDescription = newDescription;
    };

    var setImageSrc = function(src) {
        $log.info("Set imageSrc: "+src);
        $rootScope.imageSrc = src;
    };

    var metaRobotsDefault = 'noodp,noydir';
    $rootScope.metaRobots = metaRobotsDefault;
    var setMetaRobots = function(pagingService) {
        if(pagingService && pagingService.canonicalPage() && pagingService.canonicalPage() > 1) {
            $rootScope.metaRobots = metaRobotsDefault + ',noindex,follow';
        } else {
            $rootScope.metaRobots = metaRobotsDefault;
        }
    };

    var setRelPrevNextLink = function(pagingService) {
        if(pagingService) {

            $rootScope.relPrevLink = function() {
                return pagingService.previousLink() ? '/#' + pagingService.previousLink() : '';
            };
            $rootScope.relNextLink = function() {
                return pagingService.nextLink() ? '/#' + pagingService.nextLink() : '';
            };
        } else {
            $rootScope.relPrevLink = function() {
                return '';
    ...
    return {
        ...
        setRelPrevNextLink: setRelPrevNextLink
    };
  }]);

App.directive('pageMeta', ['pageMetaService', '$location', function(pageMetaService, $location) {
      return {
        restrict: 'E',
        scope: {
            pagingService: '='
        ...
        link: function(scope, elm, attrs) {
            pageMetaService.setCanonicalUrl(attrs['canonical'], attrs['pagePrefix'] ? attrs['pagePrefix'] : '', scope.pagingService, attrs['includeHash'] ? $location.hash() : null);
            pageMetaService.setTitle(attrs['title']);
            pageMetaService.setMetaDescription(attrs['meta']);
            pageMetaService.setMetaRobots(scope.pagingService);
            var image = attrs['image'] || '/images/ap/app-logo-300.png';
            pageMetaService.setImageSrc(image);
            pageMetaService.setRelPrevNextLink(scope.pagingService);
        }
      };
    }]);
